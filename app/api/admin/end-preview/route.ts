...
export async function POST(request: Request) {
  ...
  // Only update rows owned by this admin and not already ended
  ...
  if (error || !data) {
    // Return 404 — do not reveal whether the session exists but belongs to another admin
    return NextResponse.json({ error: "Preview session not found or already ended" }, { status: 404 });
  }

  console.log(`[preview] END admin=${caller.id} previewId=${previewId}`);
