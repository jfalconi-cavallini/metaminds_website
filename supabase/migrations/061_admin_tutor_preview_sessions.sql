-- ─────────────────────────────────────────────────────────────────────────────
-- 061 — Admin Tutor Preview Sessions
--
-- Allows an admin to open a tutor's portal as a read-only observer.
-- The admin remains authenticated as themselves — no impersonation occurs.
-- Mirrors admin_preview_sessions (040), scoped to tutors instead of students.
--
...
CREATE TABLE IF NOT EXISTS admin_tutor_preview_sessions (
...
CREATE UNIQUE INDEX IF NOT EXISTS admin_tutor_preview_token_idx ON admin_tutor_preview_sessions(token);
-- Admin audit queries
CREATE INDEX IF NOT EXISTS admin_tutor_preview_admin_idx ON admin_tutor_preview_sessions(admin_id);

ALTER TABLE admin_tutor_preview_sessions ENABLE ROW LEVEL SECURITY;
